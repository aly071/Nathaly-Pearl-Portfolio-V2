...
const useImageLoader = (seqRef: any, onLoad: () => void, dependencies: any[]) => {
  useEffect(() => {
    ...
    images.forEach((img: HTMLImageElement) => {
      if (img.complete) { handleImageLoad(); } else {
        img.addEventListener('load', handleImageLoad, { once: true });
        img.addEventListener('error', handleImageLoad, { once: true });
      }
    });
    return () => { images.forEach((img: HTMLImageElement) => { img.removeEventListener('load', handleImageLoad); img.removeEventListener('error', handleImageLoad); }); };
